...
- leagueStore (frontend/src/store/appStore.ts)
  ...
  - SWR: сезоны кэшируются 55 с, таблица — 30 с, расписание — 12 с, результаты — 20 с, лидерборды — 300 с, клубное резюме — 45 с; сохраняем `X-Resource-Version` и timestamp последнего fetch per season / club. При `force: true` TTL обходится.
