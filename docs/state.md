...
- `admin/src/store/adminStore.ts`: управляет аутентификацией, словарями, сезонами, матчами и статистикой. Критичные значения: TTL словарей 60 000 мс, сезонов 30 000 мс, серий 15 000 мс, матчей 10 000 мс, товарищеских матчей 45 000 мс, статистики 20 000 мс. Модуль также хранит `ads: AdBanner[]` с TTL 60 000 мс и действиями `fetchAds`, `upsertAd`, `removeAd`; после авторизации подписывается на топик `home`, где теперь приходят патчи `news.full`/`news.remove` и `ads.full`/`ads.remove`. Вкладка «Матчи» по‑прежнему полагается на TTL вместо принудительного `force`, так что лишние запросы при переключении подвкладок отсутствуют. Во вкладке «Новости» появилось управление баннерами: форма создания/редактирования вызывает `adminCreateAd`/`adminUpdateAd`, список синхронизируется через `upsertAd`/`removeAd` и веб-события.
