...
- **Состояние.** Публичный фронт получил модуль стора (`frontend/src/store/appStore.ts`) для вкладки «Лига» с TTL-кэшом и адаптивными HTTP-интервалами; результаты сезона теперь грузятся в два этапа (индекс туров + ленивые запросы матчей по `roundKey`). Остальные экраны пока опираются на локальные `useState`/`useEffect`. Админ-панель использует полноценный Zustand с TTL и ролями (admin → judge → assistant → lineup). Серверный state — через Prisma транзакции + кэш-версии в Redis.
...
**LeagueRoundsView (`frontend/src/components/league/LeagueRoundsView.tsx`)**
- **Роль.** Вкладка «Результаты» с ленивой загрузкой туров: аккордеоны по сезонам, индивидуальные спиннеры и повторные запросы по `roundKey`.
- **Фрагмент:**
```tsx
const handleToggle = async (roundKey: string, nextExpanded: boolean) => {
  onToggle(roundKey)
  if (nextExpanded) {
    await onLoadRound(roundKey)
  }
}
```
- **API/пропсы.** Принимает индекс туров, карту матчей по турам, состояния загрузки/ошибок и колбэк `onLoadRound` (обращается к `fetchLeagueResults({ roundKey })`).
- **Зависимости.** Zustand store, `leagueApi.fetchResultsRound`, локальный merge `mergeRoundCollection` для переиспользования матчей.
