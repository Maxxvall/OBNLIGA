...
* `league:results:index` — в окне: **10–20s**; вне окна: можно увеличивать до 5–10 минут, так как payload содержит только метаданные туров (ключи, счётчики матчей, временные диапазоны).
* `league:results:round:{roundKey}` — в окне: **10–20s**; вне окна: **2–5 минут**. Каждая карточка тура кэшируется отдельно, что позволяет повторно использовать уже загруженные матчи без повторного запроса всего сезона.
...
* `predictions:active:6d` — **60s** (SWR **300s**), ETag обновляется при изменении окна матчей или bump шаблонов; ключ зависит от диапазона (по умолчанию 6 суток). **Оптимизировано под Render.com Free tier — данные меняются редко, в основном после финализации матчей**.
* `predictions:user:{user_id}` — **300s** (SWR **900s**), приватный ключ с защитой по пользовательскому ID и сессии. **Личные данные кэшируются дольше для экономии Redis**.
* `predictions:legacy` — временный namespace для миграции старых записей (`Prediction`). TTL **5 min**, только для фоновых задач.

### Ratings (Рейтинги)

* `ratings:leaderboard:{scope}:p{page}:s{pageSize}` — **120s** (SWR **600s**), публичная таблица лидеров с ETag и пагинацией. **Оптимизирован для Render.com Free tier (25MB Redis, 50 подключений) — рейтинги обновляются только после финализации матчей**.
* `ratings:my:{user_id}` — **300s** (SWR **900s**), персональная сводка рейтинга пользователя.
* Redis TTL — **2 минуты** для публичных данных, **5 минут** для персональных.
* **Критично**: максимальное использование клиентского localStorage кэша для снижения нагрузки на Redis.

### Клиентское кэширование (localStorage)

**Стратегия SWR (Stale-While-Revalidate):**
- Показываем устаревшие данные пользователю мгновенно
- Обновляем данные в фоне
- Гарантируем актуальность через двухуровневые TTL:
  - `expiresAt` — время когда данные считаются устаревшими
  - `staleUntil` — время полного удаления данных

**Увеличенные TTL под Render.com Free tier:**
- Прогнозы активные: 60s fresh / 5m stale
- Мои прогнозы: 5m fresh / 15m stale  
- Рейтинги публичные: 2m fresh / 10m stale
- Мой рейтинг: 5m fresh / 15m stale

**Лимиты:**
- Прогнозы: максимум 50 записей (активные + личные)
- Рейтинги: максимум 10 комбинаций страниц/scope
- LRU очистка при превышении лимитов
- Автоматическая очистка раз в час

**Адаптивный Polling (RatingsPage) — оптимизировано под Free tier:**
- Активная вкладка: обновление каждые **60 секунд** (было 30с)
- Неактивная вкладка (видимая): каждые **5 минут** (было 2м)
- Фоновая вкладка: каждые **10 минут** (было 5м)
- Автоматическое определение активности по событиям мыши/клавиатуры/скролла

**Инвалидация:**
- После submitPrediction — автоматически сбрасывается `predictions:active`
- После admin изменений — сервер инвалидирует соответствующие ключи
- ETag гарантирует консистентность при 304 ответах

## Оптимизация под Render.com Free tier

**Ограничения:**
- Redis: 25MB памяти, 50 одновременных подключений
- База данных: Free tier PostgreSQL
- CPU/Memory: ограниченные ресурсы

**Стратегии экономии:**

1. **Максимальное использование клиентского кэша (localStorage)**
   - Уменьшает запросы к Redis и серверу
   - SWR показывает старые данные мгновенно
   - Обновление в фоне не блокирует UI

2. **Увеличенные TTL с учётом паттерна изменений**
   - Данные меняются в основном после финализации матчей
   - Между матчами данные практически статичны
   - TTL 1-5 минут вместо 15-30 секунд

3. **Адаптивный Polling снижает нагрузку**
   - Активная вкладка: 60s вместо постоянных запросов
   - Неактивная: 5m — пользователь не видит задержку
   - Фон: 10m — экономия ресурсов

4. **ETag + 304 Not Modified**
   - Сервер возвращает 304 если данные не изменились
   - Клиент обновляет только TTL локального кэша
   - Экономия трафика и CPU на сериализацию

5. **Только критичные данные в Redis**
   - ETag версии (несколько байт)
   - Предвычисленные агрегаты таблиц лидеров
   - Мониторинг: при >20MB использования — увеличить клиентский кэш
