...
- `admin/src/store/adminStore.ts`: управляет аутентификацией, словарями, сезонами, матчами, статистикой, рейтингами и рекламой. TTL: словари 60 000 мс, сезоны 30 000 мс, серии 15 000 мс, матчи 10 000 мс, товарищеские матчи 45 000 мс, статистика 20 000 мс, рекламные баннеры 60 000 мс, настройки рейтинга 60 000 мс, лидерборды 30 000 мс. Для вкладки прогнозов используется список `predictionMatches` (TTL 20 000 мс) и действия `fetchPredictionMatches`, `setPredictionTemplateAuto`, `setPredictionTemplateManual`, которые синхронизируют шаблоны через `PATCH /api/admin/matches/:matchId/prediction-template` и helper `applyPredictionTemplateOverride`. Рейтинг управляется полями `ratingScope`, `ratingPage`, `ratingPageSize` и действиями `fetchRatingSettings`, `fetchRatingLeaderboard`, `setRatingScope`, `setRatingPagination`, `updateRatingSettings`, `recalculateRatings`: первый парсит сводку настроек, второй возвращает страницы лидерборда, остальные меняют выбранный scope, обновляют пагинацию или инициируют ручной пересчёт и принудительную инвалидацию кэша. Стор подписан на топики `home` и `admin.predictions`; уведомления о рейтинге пока доставляются явными refetch-вызовами после мутаций, чтобы не зависеть от отсутствующего `admin.ratings`. Вкладка «Новости» продолжает работать через `fetchAds`, `upsertAd`, `removeAd` и веб-события. Отдельная вкладка `Ratings` уже использует эти действия: выводит актуальные окна расчёта, форму обновления с опциональным пересчётом, ручной пересчёт по наборам userId и пагинированный лидерборд с переключением `current/yearly`.
