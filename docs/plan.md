...
- Фаза 1: Приём прогнозов
  - Сервис `predictionService` для создания шаблонов на матч на основе конвейера данных; интеграция с планировщиком матчей для вычисления тоталов
  - Реализовать динамический калькулятор тоталов (считывая последние 5 матчей; запасной вариант 2.5) в `backend/src/services/predictionTotalsService.ts`
  - Открыть админ-роут для оверрайда тоталов (`PATCH /admin/matches/:id/prediction-template`)
- Фаза 2: Движок начисления очков
  - Фоновый воркер (BullMQ или существующая очередь) для разрешения прогнозов при переводе матча в статус Finished; рассчитывает скрытую систему начисления с мультипликаторами
  - Записывать результаты и отправлять WS-события инвалидации для сброса кэшей списков прогнозов
  - Поддерживать сбросы серий и обновлять `UserRating`, `PredictionStreak`, `UserAchievementProgress`
- Фаза 3: Агрегация рейтинга
  - Cron или задача в очереди каждые 6 часов для рекалькуляции рангов и обновления `RatingSnapshot`; применять сброс годовых очков 1 мая и квартальный сброс для текущего scope
...
- Фаза 4: Админ-контрольные приложения
  - Расширить админ-роуты для управления достижениями, аварийными операциями и логами; защитить ролями (role-based guards), которые уже присутствуют
  - Публиковать WS-события для админ-дашбордов, чтобы отражать корректировки в реальном времени
- Фаза 5: Интеграция фронтенда
  - Пользовательские API-эндпойнты для перечисления активных шаблонов прогнозов, отправки выборов, получения истории, достижений и рейтинга
  - Реализовать HTTP-кэширование с использованием `Last-Modified` и `ETag` плюс Redis TTL-кэши для публичных read-эндпойнтов (например, TTL 15s для таблицы лидеров, 30s для списков прогнозов). Всегда сбрасывать кэши при релевантных записях
...
- Вкладки
  ...
  - Вкладка `Predictions` показывает предстоящие матчи с вычисленными тоталами; контролы для переключения между авто/ручными тоталами и редактирования мультипликаторов сложности
...
- Поток данных
  - Переиспользовать паттерн Zustand с TTL; задать `FETCH_TTL` по области (predictions 30s, achievements 2m, emergency logs 15s). Обновлять кэши при поступлении WS-patch
  - Подписаться на новые WS-топики `admin.predictions` и `admin.achievements` для инкрементальных обновлений
...
- Вкладка прогнозов
  ...
  - Форма позволяет выбирать 1X2 и тоталы; при отправке делать `POST /predictions/:templateId/entry` с optimistic UI; скрывать текст коэффициентов (disable coefficients text) во избежание нарушения регуляций
...
- Добавить unit-тесты для scoring, расчёта тоталов, движка серий и апгрейдов достижений (jest в пакете бэкенда)
- Расширить integration tests для Fastify-рутов с моками Prisma/Redis для верификации заголовков кэширования и кодов статусов
- Создать e2e Cypress-сценарии для CRUD прогнозов и аварийного отката в админ-панели
- Выполнить миграцию БД поэтапно. Запустить скрипт backfill для инициализации рейтингов из исторических данных (если доступны) до включения production-эндпойнтов
- Обновить CI-пайплайн для запуска новых тестов и линтинга общих типов

## Операционные соображения
- Мониторинг
  - Добавить метрики для времени разрешения прогнозов, ratio попаданий в Redis, длительности вычисления таблиц лидеров, объёма WS-патчей
  - Настроить алерты по задержке очередей и падению hit rate кэша
- Откат
  - Предоставить UI `AdminActionLog` для отката корректировок в течение 72 часов путем проигрывания обратных операций
- Документация
