...
- Блок комментариев трансляции:
  ...
  - Авторизованный пользователь отображается чипом (`comment-author-chip`) с собственной аватаркой, подсказки авторизации рендерятся стеклянным блоком `comment-auth-hint`.
