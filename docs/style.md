...
- Вкладка «Профиль» (публичное приложение):
  ...
  - Блок ежедневных наград располагается сразу под карточкой участника и повторяет стеклянный стиль (градиентные дорожки, анимация `rewardPop`). Трек всегда содержит 7 дней с циклами очков `+1 / +3 / +5 / +7 / +10 / +15 / +30`, после 7-го дня цикл визуально сбрасывается, но общий стрик продолжает расти (для будущих достижений на 14–21 день).
  - Подсказку «Подтвердите статус игрока…» можно свернуть отдельной кнопкой «Скрыть». После сворачивания появляется маленькая круглая кнопка в правом верхнем углу карточки, которая возвращает подсказку; флаг хранится в `localStorage`, поэтому пользователь повторно не видит сообщение, пока сам не откроет кружок.
...
- Видео трансляции:
  ...
  - При переходе устройства в горизонтальный режим WebApp включает pseudo-fullscreen: панель чата скрывается, body получает класс `allow-landscape`. Двойной тап по плееру остаётся как shortcut к нативному fullscreen (для выхода — повторный двойной тап или системная кнопка).
...
- В верхней части `.league-sidebar` присутствует блок `.friendlies-entry` для «Товарищеских матчей»: он повторяет стеклянную карточку соревнований, подчёркивает активное состояние чипом `Активно` и визуально выравнивается с остальными группами сезонов. Карточка показывается только когда в расписании действительно есть запланированные товарищеские игры; при пустом календаре блок скрывается.
...
- `.league-round-card` (вкладка «Результаты») отображает тур как стеклянную карточку-аккордеон: по умолчанию свернут; заголовок содержит номер тура, диапазон дат и счётчик матчей. Кнопка раскрытия использует текстовые символы `+`/`-` без необычных Unicode, активное состояние выделяется неоновым бордером. Внутри раскрытого блока список матчей загружается лениво, поэтому показываем скелетон или текстовую подсказку `Раскройте тур, чтобы загрузить список матчей` до первой загрузки, а при ошибке — сообщение и кнопку повтора.
