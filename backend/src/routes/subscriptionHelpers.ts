...
/**
 * Планирует уведомления для всех предстоящих матчей команды
 * при подписке пользователя на эту команду.
 */
export async function scheduleNotificationsForClubSubscription(
  userId: number,
  telegramId: bigint,
  clubId: number
): Promise<number> {
  ...
  // Находим предстоящие матчи этой команды
  ...
  for (const match of upcomingMatches) {
    ...
    try {
      ...
      console.error('Failed to schedule notification:', { userId, matchId: match.id.toString(), err })
    }
  }
...
/**
 * Планирует уведомления для всех подписчиков команд при создании нового матча.
 * Вызывается из админки после создания матча.
 */
export async function scheduleNotificationsForNewMatch(matchId: bigint): Promise<number> {
  ...
  // Уникальные пользователи (если подписан на обе команды — одно уведомление)
  const uniqueUsers = new Map<number, { telegramId: bigint; remindBefore: number }>()
  ...
  for (const [userId, userData] of uniqueUsers) {
    const scheduledAt = new Date(match.matchDateTime)
    scheduledAt.setMinutes(scheduledAt.getMinutes() - userData.remindBefore)
