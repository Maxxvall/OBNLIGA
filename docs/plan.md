...
- Фаза 0: Схема и миграции
  - Добавить модели в Prisma, выполнить поэтапные миграции, засеять дефолтные достижения и скрытые коэффициенты для начисления очков ✅ 2025-11-03
  - Обновить общие типы новыми DTO и перечислениями (enums) ✅ 2025-11-03
...
- Фаза 2: Движок начисления очков
  ...
  - ✅ После расчёта очищается пользовательский кеш `/api/predictions/my`, поэтому статус «ожидает» снимается сразу после подтверждения исхода
...
- Фаза 3: Агрегация рейтинга
  ...
  - ✅ Активные сезоны рейтинга определяют границы окон (`startsAt`), настройки (`Параметры расчёта`) служат бэкапом и шаблоном длительности
- Фаза 4: Админ-контрольные приложения
  - Расширить админ-роуты для управления достижениями; защитить ролями (role-based guards), которые уже присутствуют ✅ 2025-11-03
- Фаза 5: Интеграция фронтенда
  - Пользовательские API-эндпойнты для перечисления активных шаблонов прогнозов, отправки выборов, получения истории, достижений (достижения будут отображаться на вкладке профиль) и рейтинга ✅ 2025-11-03
...
- Вкладка рейтинга
  ...
  - ✅ Таблица лидеров переведена в табличный макет: boundaries окон берутся из активных сезонов, отображается только выбранный диапазон очков без дублирования
...
- ✅ Кэширование на клиенте (2025-11-05)
  - ✅ Реализована система SWR (Stale-While-Revalidate) для мгновенной загрузки с фоновым обновлением
  - ✅ Двухуровневые TTL: `expiresAt` (свежие данные) и `staleUntil` (устаревшие, но показываемые)
  - ✅ Обрабатывается `304 Not Modified` с автоматическим обновлением TTL локального кэша
  - ✅ Адаптивный polling для рейтингов: активная вкладка 60с, неактивная 5м, фон 10м
  - ✅ Ограничение размера localStorage: макс 50 записей для прогнозов, 10 для рейтингов с LRU очисткой
  - ✅ Автоматическая инвалидация после мутаций (submitPrediction сбрасывает кэш активных прогнозов)
  - ✅ Circuit Breaker встроен в SWR: при ошибках показываются кэшированные данные
  - ✅ Обновлены TTL под ограничения Render.com Free: прогнозы активные 60s/5m, личные 5m/15m, рейтинги 2m/10m

## Эффективность ресурсов и стратегия кэширования
- ✅ Использование Redis (оптимизировано под Render.com Free: 25MB, 50 подключений) — 2025-11-05
  - ✅ Хранить только критичные данные: ETag версии, предвычисленные агрегаты таблиц лидеров
  - ✅ TTL оптимизированы под редкие изменения: predictions:active 60s, ratings 2m, user data 5m
  - ✅ Максимальное использование локального кэша на клиенте (localStorage) для снижения нагрузки на Redis
  - ✅ SWR стратегия: показывать устаревшие данные из кэша, обновлять в фоне — минимум запросов к Redis
  - ✅ Адаптивный polling снижает частоту запросов: активная вкладка 60s, неактивная 5m, фон 10m
  - ✅ Использовать Redis streams или pub/sub ТОЛЬКО для админ-панели, клиенты используют HTTP Polling + ETag
  - План: мониторинг использования памяти Redis, при превышении 20MB — увеличить TTL или агрессивнее использовать клиентский кэш
- Нагрузка на БД
  ...
  - ✅ Основные изменения данных происходят при финализации матча — в остальное время данные почти статичны
- ✅ HTTP-кэширование (2025-11-05)
  - ✅ Генерировать ETag на основе версии данных в Redis для эндпойнтов прогнозов и рейтингов
  - ✅ Возвращать `304 Not Modified` при совпадении `If-None-Match`, в сочетании с `Cache-Control: public, max-age=60`
  - ✅ Увеличены TTL под free tier: прогнозы активные 60s→5m stale, рейтинги 2m→10m stale, личные данные 5m→15m stale
- Эффективность WebSocket
  ...
  - ⚠️ Для публичных клиентов НЕ использовать WebSocket из-за лимита 50 подключений — только HTTP Polling + ETag
...
### 🔴 Высокий приоритет
1. **Интеграция сводки рейтинга в Profile**
   - Добавить в хедер профиля отображение текущего уровня и очков
   - CSS-обод вокруг аватара с градиентом по уровню (Bronze→Mythic)
   - Использовать данные из `GET /users/me/rating`
   
2. **Модуль достижений в профиле**
   - Раздел профиля с разблокированными уровнями достижений
   - API: `GET /users/me/achievements`
   - Кэширование 5m fresh / 15m stale
   - Отображение прогресса, иконок, описаний

3. **Вкладка Achievements в админ-панели**
   - CRUD для типов достижений
   - Управление порогами уровней (AchievementLevel)
   - Загрузка иконок через существующий upload API
   - Ручная корректировка прогресса пользователей
   - WS-топик `admin.achievements` для live-обновлений

### 🔵 Средний приоритет
4. **Вкладка Emergency в админ-панели**
   - Регулирование очков пользователей (AdminPointAdjustment)
   - Сброс серий (PredictionStreak)
   - История откатов с фильтрами (AdminActionLog)
   - Обязательные диалоги подтверждения и поле "причина"

5. **Отображение серий и подсказок в PredictionsPage**
   - Показать текущую серию пользователя (currentStreak)
   - Подсказки по достижениям ("Ещё 2 прогноза до Bronze")
   - Без раскрытия мультипликаторов начисления

6. **Мониторинг использования Redis**
   - Добавить метрики использования памяти Redis в админ-панель
   - Alert при превышении 20MB (80% от лимита)
   - Автоматическое увеличение клиентского TTL при высокой нагрузке

### ⚪ Низкий приоритет
7. **Вынести расчёт очков в фонового воркера**
   - Если нагрузка на финализацию матчей станет проблемой
   - Использовать существующую BullMQ инфраструктуру
   - Асинхронный расчёт с уведомлениями пользователей

8. **Предвычисление данных для мини-игр**
   - CDN delivery для статичных данных форм команд
   - Снижение нагрузки в дни матчей
   - Кэширование через Cloudflare или аналоги

9. **Оптимизация пула соединений Prisma**
   - Настройка connection pool для write-heavy операций
   - Рассмотреть pgbouncer для продакшена
