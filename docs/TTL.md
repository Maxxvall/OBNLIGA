...
- TTL: 10 000 мс при раскрытом чате, 30 000 мс при свёрнутом блоке комментариев или активном landscape-режиме. Интервалы задействуются только во время статуса матча `LIVE`.
- Поллинг: запускается на вкладке «Трансляция» после загрузки плеера; при скрытии вкладки браузера тик откладывается до возвращения пользователя, при смене статуса на `FINISHED` опрос прекращается после 10-секундного периода уведомления.
- Заголовок запроса: `If-None-Match: <matchDetails.commentsEtag>`.
- Ответ `200 OK`: обновлённый список комментариев сохраняется в `matchDetails.comments`, кэш синхронизируется, таймер перезапускается со свежим интервалом.
- Ответ `304`: данные не трогаются, но интервал polling продлевается; при отключении трансляции таймер очищается и запросы прекращаются.
...
## Админский клиент (`admin`)

- `fetchPredictionMatches` — TTL 20 000 мс на ключ `season:{id}`; перезагружается принудительно при ручном оверрайде тотала.
- `fetchRatingSettings` — TTL 60 000 мс на глобальный ключ; после обновления настроек и ручного пересчёта метка сбрасывается вручную, чтобы отдать свежую сводку.
- `fetchRatingLeaderboard` — TTL 30 000 мс на комбинацию `scope|page|pageSize`; любые изменения настроек или пересчёт рейтинга вызывают принудительный refetch.
- `fetchAds` — TTL 60 000 мс; при `upsertAd`/`removeAd` метка обновляется вручную, чтобы не ждать истечения.
- Остальные источники сохраняют прежние интервалы: словари 60 000 мс, сезоны 30 000 мс, серии 15 000 мс, матчи 10 000 мс, товарищеские матчи 45 000 мс, статистика 20 000 мс, достижения 120 000 мс, дисквалификации 30 000 мс.
