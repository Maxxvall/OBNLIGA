...
- `admin/src/store/adminStore.ts`: управляет аутентификацией, словарями, сезонами, матчами и статистикой. Критичные значения: TTL словарей 60 000 мс, сезонов 30 000 мс, серий 15 000 мс, матчей 10 000 мс, товарищеских матчей 45 000 мс, статистики 20 000 мс. После авторизации подключает `wsClient`, подписывается на топик `home` и моментально применяет патчи `news.full`/`news.remove`; вкладка «Матчи» теперь полагается на TTL вместо принудительного `force`, так что лишние запросы при переключении подвкладок отсутствуют.
