...
- Вкладка «Профиль» (публичное приложение):
  ...
  - Аватар и бейдж подтверждения располагаются внутри отдельной панели (padding 18–22px), под фото всегда выводится `firstName` (в верхнем регистре), ниже — `@username`, если он отличается.
  ...
  - Статистика заменена таблицей «Карьера игрока»: колонки `Год / Команда / Матчи / ЖК / КК / Пасы / Голы`, таблица лежит в стеклянном контейнере и на мобилках трансформируется в карточки через `data-label` (см. `profile-career-table`).
  - Новый чип с totals (Матчи/Пасы/Голы) выводится справа в шапке и повторяет стиль сводок «Лиги» (uppercase, letter-spacing ≥0.6px).
